...
Also adds cross-validation: `blueprint-validate` now confirms that every `emit.data` field in a state machine has a matching property in the corresponding AsyncAPI `*Data` schema, catching drift between the state machine and AsyncAPI contracts.
