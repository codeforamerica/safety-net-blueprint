...
- **Confirmed during the translator spike (#388):** this pattern already has two more real instances beyond currency. Date/age arithmetic (Corticon's `yearsBetween`) needs a calendar-aware function CEL doesn't have natively — and unlike sorting below, this one has a real Fact Graph precedent to build on: `PayrollMonthsBetween`/`DayNode`/`Today` are real, tested compnode operators in Fact Graph's own source, just for months rather than years. Collection sorting/ranking (Corticon's `sortedBy`/`first`, used to pick the best-ranked eligibility cohort among several a person qualifies for) needs an equivalent CEL doesn't have either — but here there's no Fact Graph precedent at all; no `SortedBy`/`OrderBy` compnode exists in its source. That's a genuine, domain-specific gap this design is solving without prior art to lean on, not a case of "just like currency." (Fact Graph's `Maximum`/`Minimum` operators are a narrower, better-precedented alternative worth considering over a fully generic sort primitive, if the actual need is "pick the element achieving the best value of some key" rather than arbitrary ordering.)
- **Naming philosophy for this function library:** prioritize clarity for this blueprint's actual audience (state benefits engineers) over matching either Fact Graph's or Corticon's naming conventions — neither is an authority to defer to here. Fact Graph's own tested *semantics* are worth borrowing where a precedent exists (per Decision 2's bootstrap-validation approach), independent of whether we reuse its exact names; the population who'd recognize a Fact Graph compnode name specifically is small, since Fact Graph is a niche, internally-built engine, not a widely-adopted one. Fact Graph's own acknowledged leaky API is specifically about its XML nested-operator-tree *authoring format* (see [Decision 2](#decision-2-build-a-new-engine-vs-adopting-fact-graphs-actual-runtime)) — already avoided by using CEL function calls instead of XML trees (Decision 3) — not something naming choices for individual functions could reintroduce on their own.
...
- Two real limitations, not obvious from the mapping alone: **(1)** a Ruleflow relying on iterative re-firing (rules re-evaluating as other rules' outputs change within the same pass) is not automatically untranslatable — whether it's a problem depends on what the iteration is actually doing, which the translator has to determine by analyzing the read/write attribute graph of what's inside the iterating construct, not by treating the presence of iteration itself as disqualifying. If the data only flows one direction (e.g., construct B reads a value construct A writes, but A never reads anything from B), the iteration is forward-chaining compensating for the engine having no native concept of evaluation order — this DSL's dependency graph resolves that ordering for free and the construct translates normally as an ordinary dependency, no iteration needed. Only a genuine cycle — A's logic reads a value B's logic writes, and B's logic reads a value A's logic writes, each depending on the other converging over repeated passes — has no equivalent in a single-pass dependency graph and must be detected and flagged for manual redesign rather than silently mistranslated. A genuine cycle carries a further risk worth flagging explicitly, confirmed by inspecting a real example (`Internal Rate of Return`, `github.com/corticon/corticon-classic-samples`): Corticon's own "re-execute until values stop changing" rule has no engine-level iteration cap or timeout of its own — the sample's own rule author had to build an explicit stop condition by hand (tracking every previously-tried value in a dedicated `Candidate` entity via a `newUnique` dedup check, so a repeated value produces no change and iteration halts). Nothing in Corticon guarantees termination independent of the rule author getting that guard right; a genuinely cyclic ruleset without one can loop indefinitely. This DSL's dependency graph has no equivalent failure mode — it cannot contain a cycle by construction, so evaluation always terminates — which is a real advantage of this design over a forward-chaining iterative construct, not just a translation limitation to route around. **(2)** A mechanical translation does not gain completeness/partial-evaluation semantics for free — existing rules are typically authored assuming complete data, with missing-data handling baked in as explicit conditions rather than left to the engine, since Corticon has no native concept of "unknown" for the translator to defer to. Confirmed by inspecting a real example (`Mortgage`, `github.com/corticon/corticon-classic-samples`): `Regular_NoData.ers` explicitly checks `loanapp.late30DaysSum = null` and substitutes `0` — a literal translation of that check into ordinary CEL would produce a determination this DSL's own completeness model marks fully "known," even though it's really just a guess standing in for a value nobody has actually supplied yet — exactly the "forward-chaining logic dressed up as reverse-chaining" failure mode, not a hypothetical one. The correct translation isn't to strip the check either, since the substituted value (`0`) is still needed to reproduce Corticon's original arithmetic for parity-testing purposes — it's to recognize the pattern and re-express it using this DSL's own native **Placeholder** mechanism (a default value that stands in for a not-yet-known fact while everything derived from it stays marked incomplete): the translated Fact keeps computing with `0` when the real value is absent, exactly like Corticon does, but — unlike Corticon — the DSL correctly keeps the result marked Incomplete and names what's still needed, rather than presenting a placeholder-derived guess as a final answer. Getting this right is what actually delivers the capability that motivates adopting this DSL; a translator that instead preserves these checks as literal CEL conditions gets a state onto the new engine structurally without delivering any new capability at all.
- **A genuine cycle may be rare or absent in real benefits-eligibility rule content specifically**, distinct from the general claim that Corticon's iterative construct can express one. `Internal Rate of Return` (the confirmed genuine-cycle example) is a financial root-solver, not eligibility logic — it proves the mechanism is real and Corticon can express it, not that state Medicaid/SNAP/TANF rulesets actually contain it. There's a structural reason to expect otherwise in this domain: eligibility regulations have historically been written to be computable by a caseworker working through a sequential worksheet (gross income test, then net income test, then asset test; categorical eligibility flowing one direction, e.g. TANF status feeding SNAP eligibility, not the reverse) — a rule requiring two things be solved simultaneously wouldn't have been implementable by hand long before rules engines existed. Consistent with this, the real DC Medicaid/CHIP project models a structurally identical cascading pattern (check Medicaid, then check CHIP for whoever didn't qualify) as plain sequential steps with per-rule filtering — no Branch, no Iterative flag, no cycle. The Medicaid workshop's own CHIP/Marketplace exercise uses Iterative for the same kind of cascade, but as a stylistic choice tied to routing through a Branch object, not because the underlying dependency is cyclic. This doesn't change what the translator must do — genuine cycles still need detecting and flagging when they occur — but it tempers the expectation of how often that path should actually fire against real state benefits content, versus reliably firing on non-eligibility domains like this one.
...
- **The comparison view could go further than a static diff, as a real migration aid rather than just a comprehension tool.** Several of Decision 9's flagged patterns (entity creation, service call-outs) aren't fixed by redesigning rule logic — they're fixed by changing what gets fed into this DSL's engine as input (e.g., pre-assembling Households instead of a flat Person list). A viewer that lets someone edit sample inputs interactively and see whether a flagged gap clears — not just read a static explanation of the gap — would let a state concretely work out what their own adapter/orchestration layer needs to produce, rather than inferring it from documentation. This is a real capability worth naming for whatever this tool becomes, not something the feasibility spike itself needs to build — the spike's job is proving the flagged patterns can be identified and their required input-shape changes documented; an interactive editing experience is further product work on top of that.
