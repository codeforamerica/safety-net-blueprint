...
**Checkpoint still open:** no real, populated example of an Iterative-marked construct's XML has been found anywhere public — not in either official Corticon sample repo, and not across ~88 real `.erf` files from independent third-party projects checked via GitHub code search. The Case 2 checkpoint (confirming the real XML shape) is unresolved. Building one requires either a Corticon Studio installation (blocked for now — no Progress license; a self-service 90-day trial exists at `progress.com/corticon/corticon-request-trial` but hasn't been pursued) or finding a further real example. Until resolved, case 2's untranslatable-cycle-detection logic (see [Decision 9](#decision-9-translating-between-this-dsl-and-an-existing-engines-native-rule-format)) can be designed and reasoned about, but not validated against a real captured Corticon trace the way case 1 can.
