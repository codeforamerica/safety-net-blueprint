...
| Blocking-factor lifecycle, determination state machine, event-driven re-evaluation | Domain orchestration concerns, not this DSL — see [Scope](#scope-the-rules-engine-not-the-whole-eligibility-domain). Not a from-scratch design gap: this substantially already exists as the [Eligibility domain](../domains/eligibility.md)'s `Determination`/`Decision` state machine. What's not yet designed is extending it for live partial/progressive evaluation and pre-screening. |
