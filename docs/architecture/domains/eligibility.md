...
See [Decision 2](#decision-2-determination-entity-model), [Decision 12](#decision-12-who-creates-determination-and-decision-records).
...
See [Decision 2](#decision-2-determination-entity-model), [Decision 4](#decision-4-submission-time-electronic-evaluation-scope), [Decision 12](#decision-12-who-creates-determination-and-decision-records).
...
- **Caseworker-initiated evaluation** — a caseworker can trigger the rules engine at any point during review, either against all pending Decisions for a Determination (`POST /eligibility/determinations/{id}/evaluate`) or against a single Decision (`POST /eligibility/decisions/{id}/evaluate`). Trial runs return projected outcomes without updating Decision status or firing downstream events; official runs do both. Eligibility calls `PUT /intake/applications/{id}/eligibility-snapshot` to refresh application data before each run, so the adapter receives current post-review data. See [Decision 15](#decision-15-evaluation-runs-and-history).
...
| 13 | [Application data snapshot](#decision-13-application-data-snapshot) | Intake owns a purpose-built `EligibilitySnapshot` resource per application; Eligibility calls PUT to refresh before each evaluate call; PII stays in Intake |
...
**What's being decided:** How the Eligibility rules engine receives the application data it needs for evaluation — where that data is stored across the Intake–Eligibility domain boundary, and how it gets to the adapter request body.

**Background:** If the Eligibility state machine assembled evaluation data by querying Intake's standard endpoints at evaluate time, two problems follow. First, the assembly logic — which Intake resources to query, how to join them, which fields the rules engine needs — lives in Eligibility's orchestration code rather than in the domain that owns the data; every Intake model change becomes an Eligibility change. Second, if that assembly were delegated further to the vendor adapter, there is no single point of contract coverage over what data the adapter receives: different adapter implementations could query inconsistently, cache stale data, or assemble incomplete payloads, and none of it is schema-governed. `EligibilitySnapshot` resolves both: Intake owns the assembly logic, and the adapter receives a pre-built, schema-governed payload before it is invoked. This is the commanded snapshot pattern — a cross-domain CQRS read model with consumer-driven refresh. IBM Cúram and Salesforce Government Cloud both apply an equivalent boundary — see Considerations.

**Considerations:**
- The adapter spec defines the required shapes: `IndividualDeterminationRequest` (per-member programs: member context, verification summary) and `HouseholdDeterminationRequest` (household programs: all member contexts, household data, verification summary). The state machine must be able to assemble these request bodies before calling the adapter.
- IBM Cúram calls this the **determination context** — a point-in-time snapshot of application evidence assembled for the rules engine, separate from the live intake records. Salesforce Government Cloud uses dedicated evaluation objects that carry the data through the evaluation workflow. The broader pattern is CQRS (Command Query Responsibility Segregation — separating write operations from read-optimized views) / materialized read model: the domain that needs to evaluate data maintains its own read-optimized copy.
- A stored snapshot provides a durable audit record of exactly what data was used in each evaluation — required for federal reporting, appeals, and oversight reviews.
- **PII governance:** Application data (income, household composition, demographics, assets) is PII. Storing snapshot fields on Eligibility's Determination and Decision records would copy PII into a second domain, expanding the PII surface area without a governance boundary. Bounded context isolation keeps PII in the owning domain. See [PII in cross-domain data sharing](../inter-domain-communication.md#pii-in-cross-domain-data-sharing).
- The **commanded snapshot pattern** is established across major government benefits platforms: IBM Cúram's determination context is a point-in-time snapshot of application evidence assembled for the rules engine, separate from live intake records; Salesforce Government Cloud uses dedicated evaluation objects that carry the composed data through the evaluation workflow. The owning domain materializes the view for a specific consumer; consumers call it without coupling to the source domain's internal resource structure. See also [`{ConsumerDomain}Snapshot` naming convention](../api-architecture.md#consumerdomain-snapshot-naming-convention) and [commanded snapshot pattern](../inter-domain-communication.md#commanded-snapshot-pattern).
- Three mechanisms keep the snapshot current across the full application lifecycle: (1) **At submission** — Intake creates the `EligibilitySnapshot` when seeding Determination and Decision records; required because expedited SNAP screening and Medicaid ex parte evaluation fire immediately before any caseworker action. (2) **New member added post-submission** — Intake updates the `EligibilitySnapshot` to add the new member's data. (3) **Evaluate calls** — Eligibility calls `PUT /intake/applications/{id}/eligibility-snapshot` before invoking the rules engine; Intake refreshes from current application data and returns the snapshot; the state machine uses it to assemble the adapter request body. See [Decision 15](#decision-15-evaluation-runs-and-history).

**Options:**
- **(A)** Eligibility queries Intake at evaluation time to fetch current application data before each adapter call
- **(B)** ✓ Intake maintains a purpose-built `EligibilitySnapshot` resource per application containing current household-level and per-member application data. Eligibility calls `PUT /intake/applications/{id}/eligibility-snapshot` (upsert — first PUT creates, subsequent PUTs update) at submission and at each evaluate call. Intake refreshes from current application data and returns the snapshot; the state machine uses it to assemble the adapter request body. PII stays in Intake. See [Decision 15](#decision-15-evaluation-runs-and-history) and [`EligibilitySnapshot`](intake.md#eligibilitysnapshot) in the Intake entity model.

**Decision:** Option A requires a synchronous cross-domain query inside the evaluation flow, creating a runtime dependency the adapter boundary was designed to avoid. Option B satisfies the adapter black-box constraint — the state machine assembles all data from the Intake response before the adapter call. Maintaining the snapshot in Intake rather than copying fields onto Eligibility records keeps PII in one domain and follows the commanded snapshot pattern: Intake owns the assembly logic and the adapter receives a pre-built, schema-governed payload. The upsert endpoint (`PUT`) encodes the idempotent create-or-replace semantics of a singleton sub-resource; the platform publishes `intake.eligibility_snapshot.created` or `intake.eligibility_snapshot.updated` based on the response code (201 or 200) — no explicit `emit:` step in the state machine is needed. Refreshing at each evaluate call (not continuously) avoids the complexity of event-driven synchronization while ensuring the adapter always receives post-review data. See [Decision 15](#decision-15-evaluation-runs-and-history).
...
- Medicaid ex parte evaluation is inherently per-member: it uses only that Decision's `memberSnapshot` and the electronic check results for that member. No coordination with other Decisions is required.
...
### Decision 15: Evaluation runs and history

**Status:** Decided: B

**What's being decided:** Whether trial evaluation runs during caseworker review are supported, and how the audit trail for each evaluation is maintained.

**Background:** Caseworkers commonly run the rules engine multiple times during review — correcting income figures, adding household members, resolving verifications, and re-running to see the effect before committing. The blueprint needs a way to support these intermediate runs without affecting official Decision records.

**Considerations:**
- Trial runs and official runs invoke the same rules engine with the same inputs; the difference is only what happens to the result — trial runs do not update Decision status or fire downstream events
- For SNAP, both the determination-level and per-Decision-level evaluate paths require full household data — SNAP income and resource tests apply to the household as a unit, so a per-Decision evaluate must access all members' current data via the Determination link; Medicaid MAGI evaluation is per-person and requires household composition for FPL calculation but not all members' income data
- The platform event log already records every entity lifecycle event — full fields on create, changed fields on update — queryable by subject (entity ID) and trace ID, providing a complete reconstruction of entity state at any point in time.
- Each evaluate call begins with Eligibility's state machine calling `PUT /intake/applications/{id}/eligibility-snapshot`; Intake refreshes from current application data and returns the snapshot. The platform event log records the updated `EligibilitySnapshot` entity state at each PUT — the sequence of `intake.eligibility_snapshot.updated` events across evaluate calls is the audit history of what data the rules engine received.
- IBM Cúram and Salesforce Government Cloud both support simulation/trial evaluations; both treat the platform event log (or equivalent) as the audit record rather than a separate evaluation entity.

**Options:**
- **(A)** No trial run support — evaluation only happens on the official event-driven path; caseworkers cannot preview outcomes before marking review complete
- **(B)** ✓ Add evaluate endpoints for both trial and official runs; audit history comes from entity lifecycle events on Intake's `EligibilitySnapshot` (`intake.eligibility_snapshot.updated` on each PUT); trace context propagated through snapshot refresh → rules engine → status update links the data state to the outcome in the event log

**Decision:** Option B reuses existing platform infrastructure. Each PUT to Intake's `EligibilitySnapshot` naturally produces an `intake.eligibility_snapshot.updated` event recording the updated entity state — replaying these events reconstructs the data state at any point in the review cycle. For official runs, the trace ID propagated through the snapshot refresh → rules engine → status update sequence links the data to the Decision outcomes in the event log. Trial runs return projected outcomes synchronously; they do not update `Decision.status` or fire `decision_completed` events.
...
### Decision revision and correction

The baseline treats terminal Decision states (`approved`, `denied`, `ineligible`) as final — once set, no further evaluation occurs. This is the correct default for the standard processing path.

States that need caseworkers to correct terminal Decisions — for example, overriding an ex parte auto-determination that was incorrectly resolved before caseworker review — can implement a revision path at the adapter layer: a mechanism to return a Decision to `pending` and re-evaluate it via the standard evaluate path (see [Decision 15](#decision-15-evaluation-runs-and-history)). Two constraints apply: (1) `Decision.path` must be updated to `manual` when a caseworker overrides an auto-determined result, to preserve the regulatory distinction between ex parte and caseworker-reviewed determinations required for CMS reporting; (2) entity lifecycle events on the relevant Decision and Determination provide the before-and-after audit trail for any oversight or appeals review.

### Application data snapshot refresh strategy

The baseline refreshes the `EligibilitySnapshot` on demand: Eligibility calls `PUT /intake/applications/{id}/eligibility-snapshot` before invoking the rules engine, so the adapter always receives current post-review data. States that want the snapshot to stay continuously current — rather than refreshing only at evaluate time — can configure Intake to also update the `EligibilitySnapshot` in response to `intake.application.updated` and `intake.member.updated` events. This trades higher write frequency for tighter currency guarantees; the evaluate-time refresh still applies and remains the authoritative refresh before any rules engine invocation. See [Decision 13](#decision-13-application-data-snapshot).
