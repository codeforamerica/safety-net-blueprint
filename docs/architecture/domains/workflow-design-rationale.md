...
| Task dependencies | Blocking one task on completion of another (ServiceNow, JSM, Pega) | **Planned** — a `dependsOnTaskId` field on Task, an `awaiting_prerequisite` state (`slaClock: paused`), and a system-triggered unblocking transition when the dependency completes (following the `system-resume` pattern, fired by cross-domain event wiring on `task.completed`). No cross-object guard evaluation needed. See issue #195. |
...
| Task notes / comments | User-authored notes on a task — what the caseworker did, found, or communicated (Pega case narratives, Appian case notes, ServiceNow work notes, JSM issue comments) | **Not in scope** — in benefits casework, notes belong on the case, not the task. The [Case Management](case-management.md) domain owns the case record where caseworkers document work over time. The workflow domain captures state changes and transition context via domain events and transition request bodies. |
