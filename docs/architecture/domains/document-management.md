...
- Box, Dropbox, and SharePoint all support opaque metadata bags on file objects; Documentum uses aspect metadata. None require the ECM layer to understand the business context.
...
- All five vendors (Laserfiche, Hyland OnBase, OpenText, IBM FileNet/IER, Microsoft Purview) implement the same four-state lifecycle. The naming varies — Purview uses "active/expired/deleted"; Laserfiche uses "active/cutoff/eligible for disposal/destroyed" — but the states are structurally identical.
