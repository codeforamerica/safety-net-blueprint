...
The per-section navigation state for the caseworker's review of an application, generated by the `reviewContext` sectionView composition as a composition state resource. One record per section, addressed at `/applications/{applicationId}/review-progress/{section}`. Entries are server-initialized when the application moves to `under_review`. Records are informational: they do not gate the `complete-review` transition, which states can optionally enforce via overlay. See [Decision 17](#decision-17-review-progress-is-a-composition-state-resource).
...
A read-only composite view assembled server-side for the caseworker review surface. Generated by the `reviewContext` sectionView composition at `/applications/{applicationId}/review`. Not a stored entity — assembled on request from the application's sub-resources. Exposes a section index at the root path and per-section panels at `/applications/{applicationId}/review/{section}`, each including the section's resource data alongside panel-level data (verifications, review progress state). The structure is neutral: it carries the data; the front end organizes it for whatever review workflow the state uses — per section or filtered by program.
...
| 17 | [Review-progress is a composition state resource](#decision-17-review-progress-is-a-composition-state-resource) | Per-section state generated by the reviewContext sectionView composition — navigation state kept separate from application and workflow entities. |
...
- **(B)** ✓ Server-assembled neutral composite, generated by the `reviewContext` sectionView composition at `GET /applications/{applicationId}/review` — section index at the root, per-section panels at `/review/{section}`; presentation is the front end's concern; writes still go through individual sub-resource endpoints
...
**Decision:** Review progress is a composition state resource generated by the `reviewContext` sectionView composition. Each section's progress is addressed at `/applications/{applicationId}/review-progress/{section}`. Status values (`not_started`, `in_progress`, `complete`, `flagged`) are navigation state only and overlay-extensible. Applicant-reported data is program-agnostic — program context is already on `ApplicationMember.programs`; no program dimension on review-progress.
