...
**Neutral caseworker review surface.** The `ApplicationReview` composite view assembles current application state server-side. The front end organizes it for whatever review workflow the state uses; writes go through individual sub-resource endpoints.
...
The per-section navigation state for the caseworker's review of an application, generated by the `applicationReview` sectionView composition as a composition state resource. One record per section, addressed at `/applications/{applicationId}/review-progress/{section}`. Entries are server-initialized when the application moves to `under_review`. Records are informational: they do not gate the `complete-review` transition, which states can optionally enforce via overlay. See [Decision 17](#decision-17-review-progress-is-a-composition-state-resource).
...
- `notes` — caseworker notes for this section, structured as `{ text: string, textFormat: plain | markdown | html }`, nullable. `text` holds the note content; `textFormat` controls how the client renders it (default `plain`). Scoped to the section by URL — written and read at the section's state endpoint alongside the section status. See [Decision 18](#decision-18-notes-are-a-field-on-section-review-progress).

---

### ApplicationReview

A read-only composite view assembled server-side for the caseworker review surface. Generated by the `applicationReview` sectionView composition at `/applications/{applicationId}/review`. Not a stored entity — assembled on request from the application's sub-resources. Exposes a section index at the root path and per-section panels at `/applications/{applicationId}/review/{section}`, each including the section's resource data alongside panel-level data (verifications, review progress state). The structure is neutral: it carries the data; the front end organizes it for whatever review workflow the state uses — per section or filtered by program.

Writes do not go through this endpoint. Each sub-resource retains its own write path. `ApplicationReview` is always current state; change history is via `GET /platform/events?subject={applicationId}`.
...
| 17 | [Review-progress is a composition state resource](#decision-17-review-progress-is-a-composition-state-resource) | Per-section state generated by the review sectionView composition — navigation state kept separate from application and workflow entities. |
| 18 | [Notes are a field on section review progress](#decision-18-notes-are-a-field-on-section-review-progress) | `notes` object on ReviewProgress state — text content with format flag, section-scoped by URL, no separate resource. |
...
- **(B)** ✓ Server-assembled neutral composite, generated by the `applicationReview` sectionView composition at `GET /applications/{applicationId}/review` — section index at the root, per-section panels at `/review/{section}`; presentation is the front end's concern; writes still go through individual sub-resource endpoints
- **(C)** Client-assembled — maximum flexibility; multiplied round trips; assembly logic duplicated in every consumer

---

### Decision 17: Review-progress is a composition state resource

**Status:** Decided

**What's being decided:** As a caseworker moves through an application, they need to track completion status per section (household, income, verification, etc.) — navigation state that lets them resume where they left off and signals to supervisors which sections have been reviewed. This state needs to live somewhere in the data model. Adding it to the `Application` entity conflates application data with caseworker navigation; adding it to the workflow task conflates work assignment state with review progress. The question is where per-section review status belongs and how it is exposed.
...
- Composition state resources are generated by the framework for each section of a sectionView — no hand-authored endpoint or schema needed. The section URL naturally scopes the state record.
- Caseworker notes on a section are similarly scoped to exactly one section, making them a natural fit as a field on the per-section state rather than a separate resource. See [Decision 18](#decision-18-notes-are-a-field-on-section-review-progress).

**Decision:** Review progress is a composition state resource generated by the `applicationReview` sectionView composition. Each section's progress is addressed at `/applications/{applicationId}/review-progress/{section}`. Status values (`not_started`, `in_progress`, `complete`, `flagged`) are navigation state only and overlay-extensible. Applicant-reported data is program-agnostic — program context is already on `ApplicationMember.programs`; no program dimension on review-progress.

---

### Decision 18: Notes are a field on section review progress

**Status:** Decided

**What's being decided:** Caseworkers need to capture observations during review — interview notes, section-specific comments, flags for follow-up. The question is whether section-level caseworker notes are a separate resource or a field on the per-section review progress state.

**Considerations:**
- Section notes are scoped to exactly one section — the same scope as a review progress entry. There is no cross-section note query requirement at the intake level.
- Storing notes as a field on the per-section state resource eliminates a separate endpoint and keeps section data co-located at one URL, written and read through the same composition state endpoint.
- States that need richer note management — authorship history, attachments, visibility controls — can extend the `notes` field or add a separate notes endpoint via overlay.

**Decision:** `notes` is a nullable object field on the per-section `ReviewProgress` state — `{ text: string, textFormat: plain | markdown | html }` — written and read at `/applications/{applicationId}/review-progress/{section}` alongside the section status. The `additionalProperties: true` flag on the `notes` object allows states to extend it with additional fields (visibility controls, note type, authorship) via overlay without modifying the baseline schema.
...
`ReviewProgress.notes.textFormat` accepts `plain`, `markdown`, or `html`. The baseline default is `plain`. States using a WYSIWYG note editor set `textFormat: html`; states using Markdown-based tooling set `textFormat: markdown`. No overlay change is needed — the field and all three values are present in the baseline contract. States that want to restrict permitted formats can narrow the `textFormat` enum via overlay.
