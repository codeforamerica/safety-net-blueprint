...
- **flag-expedited** — System flags a Determination as qualifying for expedited SNAP processing
  ...
  - Emit: `eligibility.determination.eligibility.application.expedited` — Intake stores the flag; Workflow switches the intake task to the expedited SLA track (7 CFR § 273.2(i))
- **complete** — System marks a Determination as complete after all program Decisions are resolved
  ...
  - Emit: `eligibility.determination.eligibility.application.determination_completed` — signals all program Decisions are resolved; Intake subscribes to close the application
...
- **approve** — System approves a Decision after automatic or caseworker review
  ...
  - Emit: `eligibility.decision.eligibility.application.decision_completed` — Intake records the outcome per member; Determination checks if all Decisions are resolved
- **deny** — System denies a Decision based on failing eligibility criteria
  ...
  - Emit: `eligibility.decision.eligibility.application.decision_completed`
