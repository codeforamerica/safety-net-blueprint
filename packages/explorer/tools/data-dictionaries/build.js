...
<body>
  ${breadcrumb([{ label: 'Explorer', href: '../../index.html' }, { label: 'Data Dictionary' }])}
  ${body}
</body>
</html>`;
}
...
function buildDomainPage(domain, sections, resolveAnn) {
  const label = domainLabel(domain);
  const body = renderDomainContent(domain, sections, resolveAnn);
  return `<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="UTF-8"/>
  <meta name="viewport" content="width=device-width, initial-scale=1.0"/>
  <title>Safety Net Blueprint — Data Dictionary — ${h(label)}</title>
  <style>${CSS}</style>
</head>
<body>
  ${breadcrumb([{ label: 'Explorer', href: '../../index.html' }, { label: 'Data Dictionary', href: 'index.html' }, { label: h(label) }])}
  ${body}
  <script>
${DOMAIN_JS}
  </script>
