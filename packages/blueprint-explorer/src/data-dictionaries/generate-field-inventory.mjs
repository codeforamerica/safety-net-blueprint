...
async function processSpec(specPath, outputPath) {
  ...
  if (specDirIsPreResolved) {
    ...
    baseUrl = resolvedSpecPath;
    specContent = readFileSync(resolvedSpecPath, 'utf8');
  } else {
    ...
    const relSpecPath = relative(specDir, resolvedSpecPath);
    const resolvedSpecFile = join(resolvedSpecDir, relSpecPath);
    baseUrl = existsSync(resolvedSpecFile) ? resolvedSpecFile : resolvedSpecPath;
    specContent = existsSync(resolvedSpecFile)
      ? readFileSync(resolvedSpecFile, 'utf8')
      : readFileSync(resolvedSpecPath, 'utf8');
  }
  ...
  try {
    ...
    rawPaths = (yaml.load(specContent, { schema: yaml.CORE_SCHEMA })).paths ?? {};

    console.log(`Loading ${specFilename}…`);
    spec = await $RefParser.dereference(
      baseUrl,
      yaml.load(specContent, { schema: yaml.CORE_SCHEMA }),
