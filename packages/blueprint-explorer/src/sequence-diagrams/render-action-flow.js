...
const configDir     = configDirArg
  ? resolve(configDirArg.slice('--config-dir='.length))
...
const ALL_SM_FILES = readdirSync(contractsDir, { recursive: true })
  .filter(f => typeof f === 'string' && f.endsWith('-state-machine.yaml'))
  .filter(f => safeResolve(contractsDir, f) !== null)
  .sort();
const SM_FILES       = ALL_SM_FILES.filter(f => !f.endsWith('platform-state-machine.yaml'));
const platformSMPath = ALL_SM_FILES.find(f => f.endsWith('platform-state-machine.yaml'));
const smDatas    = SM_FILES.map(f => yaml.load(readFileSync(resolve(contractsDir, f), 'utf8'), { schema: yaml.DEFAULT_SCHEMA }));
const platformSM = platformSMPath
  ? yaml.load(readFileSync(resolve(contractsDir, platformSMPath), 'utf8'), { schema: yaml.DEFAULT_SCHEMA })
  : { procedures: [] };
...
const domainEntries = readdirSync(contractsDir, { recursive: true })
  .filter(f => typeof f === 'string' && f.endsWith('-openapi.yaml') && !f.includes('-adapter-'))
  .filter(f => safeResolve(contractsDir, f) !== null)
