...
if (!resolvedArg) {
  console.error('Error: --resolved=<path> is required');
  process.exit(1);
}
const contractsDir = resolve(process.cwd(), resolvedArg.slice('--resolved='.length));
if (!configDirArg) {
  console.error('[validate-config] --config-dir is required');
  process.exit(1);
}
const configDir = resolve(process.cwd(), configDirArg.slice('--config-dir='.length));

// ── Load state machines ────────────────────────────────────────────────────

const SM_FILES = readdirSync(contractsDir, { recursive: true })
  .filter(f => f.endsWith('-state-machine.yaml') && !f.endsWith('platform-state-machine.yaml'))
  .sort();
