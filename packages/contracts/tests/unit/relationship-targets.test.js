...
describe('smoke tests — resolved contracts', () => {
  test('all cross-domain x-relationship targets exist in their referenced domain', { skip: !resolvedExists && 'run npm run resolve first' }, () => {
    const schemaIndex = buildCrossDomainSchemaIndex(resolvedRoot);
    const errors = [];
    for (const file of readdirSync(resolvedRoot).filter(f => f.endsWith('-openapi.yaml'))) {
      let spec;
      try { spec = yaml.load(readFileSync(join(resolvedRoot, file), 'utf8')); } catch { continue; }
      const fileErrors = validateRelationshipTargets(spec, schemaIndex);
