...
describe('smoke tests — resolved contracts', () => {
  test('all state machine emit and subscription types exist in AsyncAPI specs', { skip: !resolvedExists && 'run npm run resolve first' }, () => {
    const channelIndex = buildAsyncApiChannelIndex(resolvedRoot);
    const errors = [];

    for (const file of readdirSync(resolvedRoot).filter(f => f.endsWith('-state-machine.yaml'))) {
      let doc;
      try { doc = yaml.load(readFileSync(join(resolvedRoot, file), 'utf8')); } catch { continue; }
      const fileErrors = validateStateMachineEvents(doc, channelIndex);
      for (const err of fileErrors) errors.push(`${file}: ${err}`);
    }

    assert.deepEqual(
      errors,
      [],
      `AsyncAPI event errors:\n${errors.map(e => `  ${e}`).join('\n')}`
    );
  });
});
