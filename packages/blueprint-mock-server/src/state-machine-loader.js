 ...
 *
 * Matches by filename only (ignoring any path prefix in the extends value) so
 * that layout changes do not require updating the extends field in every file.
 *
 * @param {Object} stateMachine - Parsed state machine document
 * @param {string[]} allFilePaths - All discovered state machine file paths
 */
function resolveExtends(stateMachine, allFilePaths) {
  if (!stateMachine.extends) return;
  const extendedName = basename(stateMachine.extends);
  const extPath = allFilePaths.find(p => basename(p) === extendedName);
  if (!extPath) {
    console.warn(`extends: "${stateMachine.extends}" — "${extendedName}" not found among discovered state machine files`);
    return;
  }
  try {
...
export function discoverStateMachines(specsDir) {
  ...
  const smFiles = files.filter(f => f.endsWith('-state-machine.yaml'));
  const allFilePaths = smFiles.map(f => join(specsDir, f));
  const results = [];
  ...
  for (const file of smFiles) {
    ...
    try {
      ...
      if (Array.isArray(stateMachine.machines)) {
        resolveExtends(stateMachine, allFilePaths);
