 ...
 * to packages/mock-server/mock-data/ so the mock server always has data on startup.
 *
 * Usage:
 *   npm run mock:seed
 *   node packages/blueprint-mock-server/scripts/generate-mock-data.js [--spec=<dir>] [--out=<dir>]
 *
 * Flags:
 *   --spec=<dir>   Directory containing OpenAPI specs (default: packages/contracts/)
 *   --out=<dir>    Output directory for seed files (default: packages/mock-server/mock-data/)
...
Usage:
  node scripts/generate-mock-data.js [--spec=<dir>] [--out=<dir>] [--count=<n>]
...
`);
  ...
  for (const [apiName, data] of Object.entries(apiSeedData)) {
    const outputPath = join(options.out, `${apiName}-mock-data.yaml`);
  ...
  console.log('='.repeat(70));
  console.log(`\nStart the server with --seed=${options.out} to load the generated data.`);
