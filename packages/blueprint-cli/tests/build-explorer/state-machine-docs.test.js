...
const script      = join(explorerPkg, 'src', 'state-machine-docs', 'build.js');
const refDir      = join(explorerDir, 'state-machine-docs');

const tmpDir = mkdtempSync(join(tmpdir(), 'state-machine-docs-'));

describe('state-machine-docs', () => {
  ...
  it('all output pages match harness reference', () => {
    ...
    for (const file of readdirSync(refDir).filter(f => f.endsWith('.html'))) {
      const actual   = readFileSync(join(tmpDir, 'state-machine-docs', file), 'utf8');
      const expected = readFileSync(join(refDir, file), 'utf8');
      assert.strictEqual(actual, expected, `state-machine-docs/${file} differs from harness reference`);
    }
  });
});
