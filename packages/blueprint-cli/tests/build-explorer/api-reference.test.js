...
const script      = join(explorerPkg, 'src', 'api-reference.js');
const refDir      = join(explorerDir, 'api-reference');

const tmpDir = mkdtempSync(join(tmpdir(), 'api-reference-'));
...
describe('api-reference', () => {
  ...
  it('all output pages match harness reference', () => {
    ...
    for (const file of readdirSync(refDir).filter(f => f.endsWith('.html'))) {
      const actual   = readFileSync(join(tmpDir, 'api-reference', file), 'utf8');
      const expected = readFileSync(join(refDir, file), 'utf8');
      assert.strictEqual(actual, expected, `api-reference/${file} differs from harness reference`);
    }
  });
});
