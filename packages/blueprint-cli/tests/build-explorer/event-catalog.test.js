...
const script      = join(explorerPkg, 'src', 'event-catalog.js');
const refDir      = join(explorerDir, 'event-catalog');

const tmpDir = mkdtempSync(join(tmpdir(), 'event-catalog-'));
...
describe('event-catalog', () => {
  ...
  it('all output pages match harness reference', () => {
    ...
    for (const file of readdirSync(refDir).filter(f => f.endsWith('.html'))) {
      const actual   = readFileSync(join(tmpDir, 'event-catalog', file), 'utf8');
      const expected = readFileSync(join(refDir, file), 'utf8');
      assert.strictEqual(actual, expected, `event-catalog/${file} differs from harness reference`);
    }
  });
});
