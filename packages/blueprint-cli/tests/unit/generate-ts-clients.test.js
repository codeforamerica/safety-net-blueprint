...
`,
  ...
  describe('generateRules', () => {
    function makeDir(specsFiles, outputFiles = {}) {
      const specsDir = mkdtempSync(join(tmpdir(), 'specs-'));
      const outputDir = mkdtempSync(join(tmpdir(), 'out-'));
    ...
    it('does nothing when no graph files found', () => {
      const { specsDir, outputDir } = makeDir({
        'intake-openapi.yaml': 'openapi: 3.1.0\n',
      });
      // Should not throw
    ...
    it('generates rules.ts with evaluate() import and Rules export', () => {
      const { specsDir, outputDir } = makeDir(
        { 'eligibility-graph.yaml': 'domain: eligibility\nruleset: snapProbes\nnodes: {}\n' },
        { 'eligibility/index.ts': "export { listApplications } from './sdk.gen';\n" }
      );

      generateRules(specsDir, outputDir);

      const rules = readFileSync(join(outputDir, 'eligibility', 'rules.ts'), 'utf8');
      assert.ok(rules.includes("import { evaluate } from '@codeforamerica/blueprint-rules-engine';"),
        'should import evaluate');
      assert.ok(rules.includes("import type { SnapProbesInputs, SnapProbesResult } from './rules-types.gen.js';"),
        'should import types from rules-types.gen.js');
      assert.ok(rules.includes('export const Rules = {'), 'should export Rules');
      assert.ok(rules.includes('snapProbes: { evaluate:'), 'should include ruleset key with evaluate wrapper');
    });

    it('generates rules-types.gen.ts with FactNode-based input and result types', () => {
      const { specsDir, outputDir } = makeDir(
        { 'eligibility-graph.yaml': 'domain: eligibility\nruleset: snapProbes\nnodes: {}\n' },
        { 'eligibility/index.ts': "export { listApplications } from './sdk.gen';\n" }
      );
      ...
      const rulesTypes = readFileSync(join(outputDir, 'eligibility', 'rules-types.gen.ts'), 'utf8');
      assert.ok(rulesTypes.includes("import type { FactNode } from '@codeforamerica/blueprint-rules-engine';"),
        'should import FactNode from engine');
      assert.ok(rulesTypes.includes('export type SnapProbesInputs ='), 'should export input type');
      assert.ok(rulesTypes.includes('export type SnapProbesResult ='), 'should export result type');
    });

    it('patches domain barrel with Rules and type re-exports', () => {
      const { specsDir, outputDir } = makeDir(
        { 'eligibility-graph.yaml': 'domain: eligibility\nruleset: snapProbes\nnodes: {}\n' },
        { 'eligibility/index.ts': "export { listApplications } from './sdk.gen';\n" }
      );
      ...
      const index = readFileSync(join(outputDir, 'eligibility', 'index.ts'), 'utf8');
      assert.ok(index.includes("export { Rules } from './rules.js'"), 'should add Rules re-export');
      assert.ok(index.includes("export type { SnapProbesInputs, SnapProbesResult } from './rules-types.gen.js'"),
        'should add type re-exports');
      assert.ok(index.includes('listApplications'), 'original exports should be preserved');
    });

    it('groups multiple rulesets for the same domain into one rules.ts', () => {
      const { specsDir, outputDir } = makeDir(
        {
          'eligibility-snap-graph.yaml': 'domain: eligibility\nruleset: snapProbes\nnodes: {}\n',
          'eligibility-ma-graph.yaml': 'domain: eligibility\nruleset: maProbes\nnodes: {}\n',
        },
        { 'eligibility/index.ts': "export { listApplications } from './sdk.gen';\n" }
      );
      ...
      const rules = readFileSync(join(outputDir, 'eligibility', 'rules.ts'), 'utf8');
      assert.ok(rules.includes('snapProbes: { evaluate:'), 'should include first ruleset');
      assert.ok(rules.includes('maProbes: { evaluate:'), 'should include second ruleset');
    });

    it('skips graph files missing domain or ruleset fields', () => {
      const { specsDir, outputDir } = makeDir(
        { 'bad-graph.yaml': 'nodes: {}\n' }, // no domain or ruleset
        { 'eligibility/index.ts': "export { foo } from './sdk.gen';\n" }
      );

      generateRules(specsDir, outputDir);

      assert.ok(!existsSync(join(outputDir, 'eligibility', 'rules.ts')),
        'should not generate rules.ts for skipped files');
    });

    it('skips domains that have no generated client directory', () => {
      const { specsDir, outputDir } = makeDir({
        'eligibility-graph.yaml': 'domain: eligibility\nruleset: snapProbes\nnodes: {}\n',
      });
      // No eligibility/ directory in outputDir

      generateRules(specsDir, outputDir);

      assert.ok(!existsSync(join(outputDir, 'eligibility', 'rules.ts')),
        'should not create directory or rules.ts for missing domain');
    });

    it('does not create index.ts when it does not exist', () => {
      const { specsDir, outputDir } = makeDir({
        'eligibility-graph.yaml': 'domain: eligibility\nruleset: snapProbes\nnodes: {}\n',
      });
      mkdirSync(join(outputDir, 'eligibility'), { recursive: true });
      // No index.ts

      generateRules(specsDir, outputDir);

      assert.ok(existsSync(join(outputDir, 'eligibility', 'rules.ts')), 'rules.ts should be written');
      assert.ok(!existsSync(join(outputDir, 'eligibility', 'index.ts')), 'index.ts should not be created');
    });

    it('inlines graph data as JSON in the evaluate() call', () => {
      const { specsDir, outputDir } = makeDir(
        { 'eligibility-graph.yaml': 'domain: eligibility\nruleset: snapProbes\nnodes:\n  eligible:\n    type: output\n    expression: "true"\n' },
        { 'eligibility/index.ts': "export { foo } from './sdk.gen';\n" }
      );
      ...
      const rules = readFileSync(join(outputDir, 'eligibility', 'rules.ts'), 'utf8');
      assert.ok(rules.includes('"eligible"'), 'should inline the graph node name');
      assert.ok(rules.includes('"output"'), 'should inline the node type');

      // The inlined JSON must be parseable
      const match = rules.match(/evaluate\((\{[\s\S]*?\}), inputs\)/);
      assert.ok(match, 'should have parseable JSON argument');
      const parsed = JSON.parse(match[1]);
      assert.strictEqual(parsed.domain, 'eligibility');
      assert.strictEqual(parsed.ruleset, 'snapProbes');
    });
  });
