...
async function generateAnnotationsAndPolicies(specsDir, outputDir, annotationDomains) {
  // Walk subdirectories so annotations in domain folders (e.g. intake/intake-annotations.yaml)
  // are discovered alongside flat layouts.
  const allFiles = readdirSync(specsDir, { recursive: true })
    .map(f => (typeof f === 'string' ? f : f.toString()));

  // ── Annotations ───────────────────────────────────────────────────────────

  const annotationFiles = allFiles
    .filter(f => f.includes('-annotations') && f.endsWith('.yaml'))
    .sort();
  ...
  for (const f of annotationFiles) {
    ...
    const domain = data.domain || baseName.replace(/-annotations.*\.yaml$/, '');
...
async function main() {
  ...
  const specFiles = readdirSync(specsDir, { recursive: true }).filter(f => {
  ...
  for (const file of specFiles) {
    const domain = basename(file).replace('-openapi.yaml', '');
    domains.push(domain);
...
function collectDiscriminatorMappingKeys(obj, result = new Set(), depth = 0, maxDepth = 1000) {
  ...
  if (obj.discriminator?.mapping) {
    // Skip discriminators using JSON Schema $defs refs (#/$defs/...) — these are
    // internal to sub-schemas and are not promoted by bundleSpec into OpenAPI
    // component schemas, so hey-api cannot generate correct zod literals for them.
    const usesJsonSchemaDefs = Object.values(obj.discriminator.mapping).some(v => typeof v === 'string' && v.startsWith('#/$defs/'));
    if (!usesJsonSchemaDefs) {
