...
- **complete** — System marks a Determination as complete after all program Decisions are resolved
  ...
  - Emit: `eligibility.application.determination_completed` — signals all program Decisions are resolved; Intake subscribes to close the application
...
- **approve** — System approves a Decision after automatic or caseworker review
  ...
  - Emit: `eligibility.application.decision_completed` — Intake records the outcome per member; Determination checks if all Decisions are resolved
    - Subscribed by: [Eligibility/Determination](eligibility.md#determination), [Intake/Application](intake.md#application)
