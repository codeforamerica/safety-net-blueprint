...
/** Load a project from patterns.json -- reads sourceFile if present, otherwise falls back to raw itself (legacy { project } shape). */
function loadProjectFromClassified(raw) {
  if (raw.sourceFile) {
    const project = JSON.parse(readFileSync(raw.sourceFile, 'utf-8'));
...
function rulesheetDetailLines(rulesheet, rulesheetKey, rulePatterns, rulesheetPatterns) {
  ...
  realRules.forEach(({ rule, rawIndex: ruleRawIndex }, i) => {
    ...
    const name = `Rule ${i}` + patternSuffix;
    lines.push(`${name}:`);
    if (rule.comment) lines.push(`    # ${rule.comment.text}`);
    ...
    // with nothing above it to complete. All actions are indented under the IF/THEN:
    // header so it's clear the whole block is gated by the same conditions.
    if (conditions) {
...
function renderDiagram(project, classification, context, rulesheetPatterns, rulePatterns, { skipStrips = false } = {}) {
...
/**
 * Load patterns.json, run the full render pipeline, and return a raw `<svg>`
 * element string suitable for embedding inline in another HTML document.
 * markerId scopes the arrowhead marker so it doesn't collide with other SVGs on
 * the same page.
 */
