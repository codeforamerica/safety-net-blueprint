...
function printUsage() {
  ...
  console.error('  <project.translated.json> is the output of: node src/translate-project.js <project.patterns.json> --out <project.translated.json>');
  console.error('  --out defaults to generated/fact-graph.html');
  console.error('Example: node src/translate-project.js generated/all-patterns.patterns.json --out generated/all-patterns.translated.json');
...
function renderDiagram(facts, { skipStrips = false } = {}) {
  ...
  if (!skipStrips && writableFacts.length) {
    const LINE_H = 15;
    const COL_W = 380;
