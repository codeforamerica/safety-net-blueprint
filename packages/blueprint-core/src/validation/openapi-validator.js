 ...
 * @param {boolean} options.skipExamples - Skip example validation (default: false)
 * @returns {Promise<ValidationResult>} Validation result
 */
export async function validateSpec(specPath, { skipExamples = false } = {}) {
  const errors = [];
  const warnings = [];
 ...
 * @param {Object} options - Validation options forwarded to validateSpec
 * @returns {Promise<Object>} Validation results keyed by API name
 */
export async function validateAll(apiSpecs, options = {}) {
  const results = {};

  for (const api of apiSpecs) {
    const specResult = await validateSpec(api.specPath, options);
    results[api.name] = {
      spec: specResult,
