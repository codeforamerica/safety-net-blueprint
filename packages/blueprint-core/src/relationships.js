...
function resolveExternalDefRef(ref, currentResults) {
  ...
  let filePart = ref.slice(0, hashIdx).replace(/^\.\//, '');
  const anchorPart = ref.slice(hashIdx + 1); // e.g. '/$defs/ApplicationMember'

  // Direct lookup first. If not found and the ref has leading ../ segments (i.e. the
  // spec lives in a subdirectory but currentResults keys are root-relative), strip the
  // leading ../ segments and retry. This handles nested directory structures where
  // intake/intake-openapi.yaml refs ../schemas/domain/intake.yaml but the map key is
  // schemas/domain/intake.yaml.
  let spec = currentResults.get(filePart);
  if (!spec && filePart.startsWith('../')) {
    const normalized = filePart.replace(/^(\.\.\/)+/, '');
    const candidate = currentResults.get(normalized);
    if (candidate) { spec = candidate; filePart = normalized; }
  }
  if (!spec) return {};
