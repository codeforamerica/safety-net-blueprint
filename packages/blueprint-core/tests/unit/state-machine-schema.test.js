 ...
 * Smoke tests against real safety-net-contracts files live in blueprint-cli/tests/state-machine-schema.test.js.
 */
...
function makeValidator() {
  ...
  const platformEventsPath = join(contractsRoot, 'base-contracts/schemas/events.yaml');
  const platformEvents = yaml.load(readFileSync(platformEventsPath, 'utf8'));
  ajv.addSchema(platformEvents, 'base-contracts/schemas/events.yaml');
  const commonEnumsPath = join(contractsRoot, 'base-contracts/schemas/enums.yaml');
  const commonEnums = yaml.load(readFileSync(commonEnumsPath, 'utf8'));
  ajv.addSchema(commonEnums, 'base-contracts/schemas/enums.yaml');
...
test('state-machine-schema action types', async (t) => {
  ...
  await t.test('action with full transition and guards', () => {
    const doc = withActions([{
      id: 'activate',
      guards: [{ actors: ['readwrite'], conditions: ['callerIsCaseworker'] }],
      transition: { from: 'draft', to: 'active' },
      steps: [
  ...
  function docWithGuards(conditions) {
    return {
      ...
      machines: [{
        ...
        actions: [{
          id: 'do-thing',
          guards: [{ actors: ['readwrite'], conditions }],
