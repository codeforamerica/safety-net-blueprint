...
describe('validateWithinFile — transition state references', () => {
  test('passes when transition states exist', () => {
    const doc = makeDoc({
      actions: [{
        id: 'submit',
        guards: [{ actors: ['readonly'], conditions: ['callerIsApplicant'] }],
  ...
  test('errors when condition ID is not declared', () => {
    const doc = makeDoc({
      actions: [{
        id: 'submit',
        transition: { from: 'draft', to: 'submitted' },
        guards: [{ actors: ['readonly'], conditions: ['undeclaredGuard'] }],
      }],
    });
  ...
  test('passes when condition uses {any: [...]} form with valid guards', () => {
    const doc = makeDoc({
      actions: [{
        id: 'submit',
        transition: { from: 'draft', to: 'submitted' },
        guards: [{ actors: ['readonly'], conditions: [{ any: ['callerIsApplicant', 'callerIsCaseworker'] }] }],
      }],
    });
    assert.equal(validateWithinFile('/fake/path.yaml', doc).length, 0);
  });
  ...
  test('errors when {any: [...]} references undeclared guard', () => {
    const doc = makeDoc({
      actions: [{
        id: 'submit',
        transition: { from: 'draft', to: 'submitted' },
        guards: [{ actors: ['readonly'], conditions: [{ any: ['callerIsApplicant', 'unknownGuard'] }] }],
      }],
    });
    assert.ok(hasRule(validateWithinFile('/fake/path.yaml', doc), 'unknown-guard'));
  });
  ...
  test('passes with valid actor roles', () => {
    const doc = makeDoc({
      actions: [{
        id: 'submit',
        transition: { from: 'draft', to: 'submitted' },
        guards: [{ actors: ['readonly', 'readwrite', 'admin', 'system'], conditions: ['callerIsApplicant'] }],
      }],
    });
    assert.equal(validateWithinFile('/fake/path.yaml', doc).length, 0);
  });
